...
import { b as useRouter$1, c as createError, u as useHead, n as navigateTo } from './server.mjs';
...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><div class="block text-base md:text-lg font-medium description-lembaga"><div class="flex py-3 border-b border-slate-300"><div class="flex-none w-[140px] sm:w-[200px]"> Nama Lembaga </div><div> : ${ssrInterpolate(unref(data).name)}</div></div><div class="flex py-3 border-b border-slate-300"><div class="flex-none w-[140px] sm:w-[200px]"> Singkatan </div><div> : ${ssrInterpolate(unref(data).surname)}</div></div><div class="flex py-3 border-b border-slate-300"><div class="flex-none w-[140px] sm:w-[200px]"> Alamat Lengkap </div><div> : ${ssrInterpolate(unref(data).address)}</div></div></div></div><div class="mt-8 text-lg sm:text-xl">`);
      if (unref(data).profile) {
        _push(`<div class="border border-[#0088CC] rounded-md"><div class="bg-[#0088CC] text-white px-2 py-3 font-medium rounded-t-md"> Profil Lembaga </div><div class="px-3 py-5 text-[14px] quill-content">${unref(data).profile}</div></div>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(data).visi) {
        _push(`<div class="border border-[#0088CC] rounded-md mt-8"><div class="bg-[#0088CC] text-white px-2 py-3 font-medium rounded-t-md"> Visi &amp; Misi </div><div class="px-3 py-5 text-[14px] quill-content">${unref(data).visi}</div></div>`);
      } else {
        _push(`<!---->`);
      }
      if (unref(data).tugas) {
        _push(`<div class="border border-[#0088CC] rounded-md mt-8"><div class="bg-[#0088CC] text-white px-2 py-3 font-medium rounded-t-md"> Tugas </div><div class="px-3 py-5 text-[14px] quill-content">${unref(data).tugas}</div></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div></div>`);
...
//# sourceMappingURL=_id_-978bdd03.mjs.map
